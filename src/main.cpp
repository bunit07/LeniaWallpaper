...
bool App::Init(HINSTANCE inst) {
    inst_ = inst;
    Config::WriteDefaultIfMissing(ExeDir() / "config.json");
    config_.Load(ExeDir() / "config.json");
    ...
    cb.onReseed = [this] {
        for (auto& s : surfaces_) {
            s->renderer.ReseedCurrent(false, rng_);
            s->renderer.StepAndPresent();
            s->consecutiveDeadChecks = 0;
            s->rotateDeadlineMs = 0;
        }
    };
    cb.onRandomSoup = [this] {
        for (auto& s : surfaces_) {
            s->renderer.ReseedCurrent(true, rng_);
            s->renderer.StepAndPresent();
            s->consecutiveDeadChecks = 0;
            s->rotateDeadlineMs = 0;
        }
...
bool App::LoadSpeciesOnto(MonitorSurface& s) {
    ...
    if (!config_.species.empty()) {  // pinned species from config.json (all monitors)
        std::string err;
        picked = ParseSpecies(catalogPath_ / Utf8ToWide(config_.species), &sp, &err);
        if (!picked) LogLine("pinned species %s: %s", config_.species.c_str(), err.c_str());
    }
    if (!picked && !catalog_.PickRandom(rng_, config_.maxKernelRadius, &sp)) {
        LogLine("no loadable species found");
        return false;
    }
    AssignDisplayName(sp);
    if (!s.renderer.LoadSpecies(sp, rng_)) {
...
void App::UpdateTraySpecies() {
    ...
    for (const auto& s : surfaces_) {
        if (s->renderer.HasSpecies()) names.push_back(s->renderer.Current().name);
    }
